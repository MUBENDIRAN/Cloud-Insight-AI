...
const S3_REPORT_URL = config.reportUrl;
...
function updateLogLevels(levels) {
  ...
  setupLogLevel(elements.criticalCount, criticalCount, () =>
    showErrorDetails("critical"),
  );
  setupLogLevel(elements.errorCount, errorCount, () =>
    showErrorDetails("error"),
  );
  setupLogLevel(elements.warningCount, warningCount, () =>
    showWarningDetails("warning"),
  );
  setupLogLevel(elements.infoCount, infoCount, () =>
    showInfoDetails(infoCount),
  );
}
...
function showWarningDetails(severity) {
  ...
  const warnings = DashboardStore.data.warning_details.filter(
...
function updateTopErrors(errorDetails) {
  ...
  if (!errorDetails || errorDetails.length === 0) {
    topErrorsEl.innerHTML =
      '<p style="text-align: center; color: var(--success-color); padding: 2rem;">✅ No errors</p>';
    return;
  }
...
function updateCostBreakdown(costBreakdown, costMetadata) {
  if (elements.costBreakdownSubtitle && costMetadata) {
    elements.costBreakdownSubtitle.textContent = `Billing Period: ${costMetadata.billing_period} | Account: ${costMetadata.account} (${costMetadata.region})`;
  }
